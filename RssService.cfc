...
<cffunction name="getXML" access="private" returnType="any" output="true"
	...
	<!--- <cfset arguments.data = "http://spanishwizards.com/fakeurl" /> --->
	...
	<cftry>
		<cfif isSimpleValue(trim(arguments.data)) and isXml(trim(arguments.data))>
			<cfreturn xmlParse(arguments.data)>
		<cfelse>
			<cfreturn xmlParse(getDefaultXml())>
		</cfif>
	    <cfcatch>
			<cfoutput>#htmlCodeFormat(arguments.data)#</cfoutput><cfdump var="argument: #arguments.data#" /><cfdump var="#cfcatch#" /><cfdump var="#cgi#" /><cfabort>
		</cfcatch>
	</cftry>
	
	<cfthrow type="rss.cfc" message="The data passed was not a valid XML object, string, or URL pointing to XML., #arguments.data#">
...
<cffunction name="getDefaultXml" access="private" returnType="any" output="false"
			hint=""> 
	<cfset var defaultXml = "" />
	<cfsavecontent variable="defaultXml">
	 <?xml version="1.0"?>
	 <data>
	  <display>Nothing available</display>
	 </data>	
	</cfsavecontent>	
	<cfreturn trim(defaultXml) />
</cffunction>
