<br style="clear: both" />
 <!-- footer begin -->
 <div id="footercontainer">
  <ul id="footernav">
   <li><a href="index.cfm">Welcome</a> |</li>
   <li><a href="about_therapy.cfm">About Therapy</a> |</li>
   <li><a href="specializations.cfm">Specializations</a> |</li>
   <li><a href="why_holly.cfm">Why Holly</a> |</li>
   <li><a href="resources.cfm">Resources</a> |</li>
   <li><a href="insurance.cfm">Insurance</a> |</li>
   <li><a href="contact.cfm">Contact</a> |</li>
   <li><a href="add_to_favorites.cfm">Add to Favorites</a> |</li>
   <li><a href="professional_opinions.cfm">Professional Opinions</a></li>
  </ul>
 <span class="disclaimer">Site by <a href="mailto:bmfitzgerald3@yahoo.com">Brian FitzGerald</a> | Holly FitzGerald Clinical Social Work is a trademark of Holly FitzGerald. Images and content used throughout this Web site may not be used without written consent. Other logos and images used belong to their respective owners and are used with permission.</span> 
 </div>
 <!-- footer end -->
</div>
</center>
<div id="extraDiv2"><span></span></div>
<script src="http://www.google-analytics.com/urchin.js" type="text/javascript">
</script>
<script type="text/javascript">
_uacct = "UA-3563632-2";
urchinTracker();
</script>
</body>
</html>